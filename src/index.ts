...
const maxContext = Number.parseInt(
  process.env.MAX_CONTEXT_MESSAGES || "500",
  10
);
...
const autosummaryEnabled =
  String(process.env.AUTOSUMMARY_ENABLED || "false").toLowerCase() === "true";
const autosummaryCron = process.env.AUTOSUMMARY_INTERVAL_CRON || "0 21 * * *";
const autosummaryMin = Number.parseInt(
  process.env.AUTOSUMMARY_MIN_MESSAGES || "25",
  10
);
const autosummaryTarget = process.env.AUTOSUMMARY_TARGET_CHANNEL_ID || null;
const autosummaryLookbackHrs = Number.parseInt(
  process.env.AUTOSUMMARY_LOOKBACK_HOURS || "6",
  10
);
...
client.once("ready", async () => {
  ...
  try {
    ...
    const perms = Number.parseInt(
      process.env.INVITE_PERMISSIONS || "84992",
      10
    );
    if (clientId) {
...
async function sendLong(channel: TextChannel, content: string): Promise<void> {
...
async function deliverInteractionText(
...
async function backfillChannelMessages(
  ...
  try {
    ...
    while (collected.length < maxFetch) {
      ...
      if (oldest && oldest.createdTimestamp < sinceMs && batch.length === 0)
        break;
      for (const msg of batch.sort(
...
async function summarizeChannel(
  ...
  if (notes.length) {
    ...
    for (const [section, arr] of Object.entries(grouped)) {
      const lines = arr.map(
        (n) => `- [${fmt(n.timestamp)}] ${n.author}: ${n.text}`
      );
      parts.push(`**${section}**\n${lines.join("\n")}`);
    }
    notesSection = `\n\n${parts.join("\n\n")}`;
  ...
  const happeningsBlock = renderHappenings({
    sinceISO: start.toISO(),
    untilISO: end.toISO(),
  });

  return {
    title: `#${channel?.name || chId}`,
    content: summary + footer + notesSection + happeningsBlock,
  };
}
...
async function runDigestOnce(): Promise<void> {
  try {
    ...
    const hours = Number.parseInt(
      process.env.SUMMARY_HOURS_DEFAULT || "24",
      10
    );
    const { start, end } = windowHoursEnd(tz, hours);
    ...
    const universeHappenings = renderHappenings({
      sinceISO: start.toISO(),
      untilISO: end.toISO(),
    });
    ...
    const finalContent = [header, ...sections, universeHappenings]
      .filter(Boolean)
      .join("\n\n--- \n\n");
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (!allowed.has(chId)) {
    await interaction.reply({
      ephemeral: true,
      content: "This channel is not allowlisted.",
    });
    return;
  }

  const hoursDefault = Number.parseInt(
    process.env.SUMMARY_HOURS_DEFAULT || "24",
    10
  );
  ...
  if (name === "summary" || name === "daily" || name === "report") {
    ...
    if (name === "report") {
      ...
      await deliverInteractionText(
        interaction,
        `**Purrfect Universe – Channel Report**\n\n${desc}`
      );
      return;
    }
  ...
  if (name === "backfill") {
    ...
    await interaction.editReply(
      `Backfill complete: stored ${fetched.length} messages from #${channel.name} (last ${hours}h).`
    );
  }
});
...
startServer({
  port: Number.parseInt(
    process.env.SERVER_PORT || process.env.STATUS_PORT || "3000",
    10
  ),
  ...
  onHappening: async (happening) => {
    await addNote({
      ...
      authorId: "dashboard",
      channelId:
        happening.channelId || process.env.DISCORD_SUMMARY_CHANNEL_ID || null,
      section:
        happening.section ||
        process.env.KEYHAPPENINGS_SECTION_NAME ||
        "Key Happenings",
      text: happening.text || "",
    });
  },
  ...
  getState: async () => {
    ...
    for (const chId of Array.from(allowed)) {
      ...
      const c24 = loadWindow({
        channelId: chId,
        sinceISO: s24.toISO(),
        untilISO: e24.toISO(),
      }).length;
      const c7d = loadWindow({
        channelId: chId,
        sinceISO: s7d.toISO(),
        untilISO: e7d.toISO(),
      }).length;
