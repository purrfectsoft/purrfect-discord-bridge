...
import renderDashboard, {
	render_metrics,
} from "./dashboard/render_dashboard.js";
...
function hxSnippet(
	message: string,
	{ variant = "info" }: { variant?: HxVariant } = {},
): string {
...
export function startServer({
	...
	const server = http.createServer(async (req, res) => {
		try {
			...
			if (req.method === "GET" && url.pathname === "/health.json") {
				...
				return res.end(
					JSON.stringify({ ok: true, nowISO: new Date().toISOString(), ...st }),
				);
			}
			...
			if (req.method === "POST" && url.pathname === "/note") {
				...
				const { text, section, channelId, author, authorId, timestampISO } =
					body;
				...
				if (!chId || !isAllowedChannel(chId)) {
					...
					res.end(
						JSON.stringify({
							ok: false,
							error: "invalid or not-allowlisted channelId",
						}),
					);
					return;
				}
					...
					onNote({
						...
						authorId:
							typeof authorId === "string" && authorId.trim()
								...
								: "dashboard",
						timestampISO:
							typeof timestampISO === "string" && timestampISO.trim()
								? timestampISO
			...
			if (req.method === "POST" && url.pathname === "/happening") {
				...
				const { text, section, author, authorId, timestampISO, channelId } =
					body;
				if (typeof text !== "string" || !text.trim()) {
					if (hx) {
						sendHx(
							res,
							200,
							"Text is required to record a happening.",
							"error",
						);
						return;
					}
				...
				if (typeof onHappening === "function") {
					await Promise.resolve(
						onHappening({
							...
							timestampISO:
								typeof timestampISO === "string" ? timestampISO : undefined,
	...
	server.listen(port, host, () => {
		console.log(
			`📊 Unified dashboard+webhook on http://${host}:${port}  (/, /health.json, /note, /happening, /digest)`,
		);
		if (canonicalBaseUrl) {
